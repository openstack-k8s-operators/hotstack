#!/bin/env python3
# md5sum="a4e23707e06fc45393bd5aad3b1c212b"
...
class POAPHandler:
    ...
    def gen_ssh_key_2048(self):
        """Generate a 2048-bit SSH key"""
        parts = []
        parts.append("terminal dont-ask")
        parts.append("configure terminal")
        parts.append("ssh key rsa 2048 force")
        parts.append("end")
        cmd = " ; ".join(parts)
        self._log("SSH Key gen command: %s" % cmd)
        try:
            cli(cmd)
        except Exception as e:
            exc_type, exc_value, exc_tb = sys.exc_info()
            ...
            self._log("SSH Key gen failed, Traceback: %s" % traceback_str)

        self._log("SSH Key gen completed successfully")
