# Devstack with Cisco NXOS VLAN Trunking (RESTCONF OpenConfig)

Single-switch topology with 1 Cisco NX-OS 9000v switch, 1 Devstack
node, 2 Ironic nodes, and 1 controller. Uses
networking-generic-switch with the `restconf_openconfig` driver for
RESTCONF-based switch management.
...
switch bootstraps itself at first boot with NX-API (RESTCONF) and
OpenConfig enabled.
...
```bash
ansible-playbook \
  -e @scenarios/networking-lab/devstack-nxos-vlan-restconf/bootstrap_vars.yml \
...
- `feature nxapi` and `nxapi http port 80` for RESTCONF access
- `feature openconfig` for OpenConfig YANG models
...
`restconf_openconfig` device type. RESTCONF uses HTTP via the
NX-API interface on port 80. NX-OS specific content type
(`application/yang.data+json`) and port ID regex substitution
are configured. VLAN membership on trunk and access ports is
managed dynamically by NGS.
