...
- Configure networking-generic-switch with leaf switch credentials (via local.conf)
- Enable L2VNI trunk reconciliation for automatic VLAN management (via local.conf)
...
This scenario includes the L2VNI trunk reconciliation feature from networking-baremetal, which automatically manages trunk subports on the devstack network node. This enables:

- Automatic VLAN management on the trunk port connecting to the physical switch
- Dynamic addition/removal of VLANs as overlay networks are created/deleted
- Synchronization with OVN ha_chassis_group membership
...
- **L2VNI Trunk Reconciliation**: Test automatic trunk port management for network nodes
