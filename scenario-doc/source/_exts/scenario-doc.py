...
class HotStackDirective(rst.Directive):
    ...
    def add_template_manifest(self, automation_dir, manifest, node):
        ...
        node += literal(text="Apply the manifest:")
        node += literal_block(text="oc apply -f {}".format(manifest_name))

    @staticmethod
    def add_wait_conditions(conditions, node):
        node += paragraph(
            text="Check that the resources was successfully " "created, and is ready."
        )
    ...
    def add_heat_template(self):
        if self.heat_template is not None:
            ...
            text = (
                "This heat template can be used with the roles in "
    ...
    def run(self):
        ...
        for path in automation_vars_paths:
            ...
            for stage in automation_vars["stages"]:
                toctree.append(
                    self.state.document, item=self.process_stage(automation_dir, stage)
                )
