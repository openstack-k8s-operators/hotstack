# Copyright Red Hat, Inc.
# All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License"); you may
# not use this file except in compliance with the License. You may obtain
# a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
# WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the
# License for the specific language governing permissions and limitations
# under the License.

import base64
import os
import sys
import yaml

from ansible.module_utils.basic import AnsibleModule
from ansible.module_utils import errors as ansible_exc

try:
    import openstack
    from openstack import exceptions as os_exc

    HAS_OPENSTACK = True
except ImportError:
    HAS_OPENSTACK = False


ANSIBLE_METADATA = {
    "metadata_version": "1.1",
    "status": ["preview"],
    "supported_by": "community",
}

DOCUMENTATION = r"""
---
module: ocp_agent_installer_snapshot

short_description: Create snapshot resource set from instances
version_added: "2.8"

description:
    - Create snapshot resource set from instances

options:
  cloud:
    description:
      - Openstack cloud name
  instances:
    description:
      - Instances to snapshot
    type: dict

author:
    - Harald Jensås <hjensas@redhat.com>
"""

EXAMPLES = r"""
- name: Create snapshot of instances and volumes
  hotloop_oc_apply_file:
    instances:
      controller:
        hotstack_role: controller
        uuid: 6f4512de-f744-4979-8ab2-45f5461e304c
        mac_address: "fa:16:9e:81:f6:5"
      master0:
        hotstack_role: ocp_master
        uuid: 6f4512de-f744-4979-8ab2-45f5461e304c
        mac_address: "fa:16:9e:81:f6:10"
"""

RETURN = r"""
ocp_snapshot_set:
  controller_image: <uuid>
  master0_image: <uuid>
"""

INSTANCES_REQUIRED_KEYS = {
    "role",
    "uuid",
    "mac",
}

SERVER_SHUTOFF = "SHUTOFF"


def _create_image_from_server(conn, name, uuid, role, mac_address, uniq):
    """Create image from server

    This function creates an image from an existing server using the
    OpenStack Nova API. It tags the newly created image with 'name',
    'role', 'uniq_id', and 'mac_address' for easy identification.

    :param conn: openstack connection
    :param name: name of server
    :param uuid: uuid of server
    :param role: role of server
    :param mac_address: mac address of server
    :param uniq: unique id
    :return: image id
    """
    image_name = "hotstack-" + name + "-snapshot-" + uniq
    image = conn.compute.create_server_image(uuid, image_name, wait=True, timeout=120)
    conn.image.add_tag(image, "name=" + name)
    conn.image.add_tag(image, "role=" + role)
    conn.image.add_tag(image, "uniq_id=" + uniq)
    conn.image.add_tag(image, "mac_address=" + mac_address)

    return image.id


def create_snapshot_set(conn, instances):
    """Create snapshot resource set from instances

    :param conn: openstack connection
    :param instances: instances to snapshot
    :return: snapshot resource set
    """
    uniq = base64.urlsafe_b64encode(os.urandom(6)).decode("utf-8")
    snapshot_set = dict()
    snapshot_set["snapshot_id"] = uniq

    for name, data in instances.items():
        uuid = data["uuid"]
        role = data["role"]
        mac_address = data["mac_address"]

        image_id = _create_image_from_server(conn, name, uuid, role, mac_address, uniq)
        snapshot_set[name] = {
            "image_id": image_id,
            "role": role,
            "mac_address": mac_address,
            "uniq_id": uniq,
        }

    return snapshot_set


def validate_server_state(servers):
    """Validate that all server are in the required SHUTOFF state.

    :param servers: list of servers to validate
    :raises: AnsibleValidationError - if any of the servers are not in the required state
    """
    for server in servers:
        if server.status != SERVER_SHUTOFF:
            raise ansible_exc.AnsibleValidationError(
                "instance {server} is not in the {required_state} state".format(
                    server=server.id, requiered_state=SERVER_SHUTOFF
                )
            )


def get_instances(conn, instances):
    servers = list()
    for k, v in instances:
        try:
            servers.append(conn.compute.get_server(v["uuid"]))
        except os_exc.ResourceNotFound:
            raise ansible_exc.AnsibleValidationError(
                "instance {instance} is not found in the openstack cloud".format(
                    instance=v["uuid"]
                )
            )


def validate_parameter_instances(instances):
    """Validates the instances paramter data structure and content.

    :param instances: The input data structure.
    :raises: AnsibleValidationError - if the input is invalid
    """
    if not isinstance(instances, dict):
        raise ansible_exc.ArgumentTypeError("instances must be a dict")

    for k, v in instances.items():
        if not isinstance(v, dict):
            raise ansible_exc.ArgumentTypeError(
                "instance must be a dict {instance} is type: {v_type}".format(
                    instance=k, v_type=type(v)
                )
            )

        if v.keys() - INSTANCES_REQUIRED_KEYS:
            raise ansible_exc.AnsibleValidationError(
                "instance {instance} is missing required keys: {keys}".format(
                    instance=k, keys=v.keys() - INSTANCES_REQUIRED_KEYS
                )
            )

        for key in INSTANCES_REQUIRED_KEYS:
            if not isinstance(v[key], str):
                raise ansible_exc.AnsibleValidationError(
                    "invalid instance value for {instance}, {key} must be a string".format(
                        instance=k, key=key
                    )
                )


def run_module():
    argument_spec = yaml.safe_load(DOCUMENTATION)["options"]
    module = AnsibleModule(argument_spec, supports_check_mode=False)

    if not HAS_OPENSTACK:
        module.fail_json(
            msg='Could not import "openstack" library. \
              openstack is required on PYTHONPATH to run this module',
            python=sys.executable,
            python_version=sys.version,
            python_system_path=sys.path,
        )

    result = dict(
        success=False,
        changed=False,
        snapshot_set=dict(),
        error="",
    )

    cloud = module.params["cloud"]
    instances = module.params["instances"]

    try:
        validate_parameter_instances(instances)

        conn = openstack.connect(cloud)
        servers = get_instances(conn, instances)
        validate_server_state(servers)
        snapshot_set = create_snapshot_set(conn, instances)

        result["snapshot_set"] = snapshot_set
        result["success"] = True
        result["changed"] = True

        module.exit_json(**result)
    except Exception as err:
        result["error"] = str(err)
        result["msg"] = "Failed to create snapshots from instances, {err}".format(
            err=err
        )
        module.fail_json(**result)


def main():
    run_module()


if __name__ == "__main__":
    main()
